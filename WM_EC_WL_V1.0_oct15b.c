...
SoftwareSerial Serial2(8,9); //SIM800L Tx=8, Rx=9
...
ISR(WDT_vect) {
  if(f_wdt == 0) {
    ...
    if(count == 10){// report every 2 min (150 is a 4hrs delay?)
      f_wdt=1;
      count=0;
    }
    else
...
void SENSOR_READINGS(){
 ...
 // EC Sensor Reading SATE 
  ...
  if (WaterResistance < 300.0) //Low conductivity solutions
  {
    ...
    WaterResistance = WaterResistance*0.94;
    WaterConductivity = compConduc(RawConduc(WaterResistance), temperature());
...
void SIM_GPRS() {
  ...
  switch (SIM_state)
  {
    ...
    //Get reading from four sensors
    case SENSOR:
      
      InitialTime = millis();
      Serial.print(" Sensors Readings Initial Time in Seconds: ");
      Serial.println(InitialTime/1000);
      ...
      delay(1000); //1 more second then OFF, approx 10 seconds ON
    ...
    case SIMOFF: //  Sleep Mode
      ...
      */
      ...
      Serial.println((millis() - InitialTime)/1000);
