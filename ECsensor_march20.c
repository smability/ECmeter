...
float area = (1.02*.9);  //in cm2 (total area of the two electrodes?)
...
float differential(){
  ...
  float voltageV1 = ValueA0 * (5.0 / 1023.0)+offset; // V1 voltage
  float voltageV2 = ValueA1 * (5.0 / 1023.0)+offset; // V2 voltage
...
void loop(void)
{
  ...
  float Vin_avg = 3.48; //0.22v is the offset, 0.03v is cable loss
  ...
  float Kcomp25;
  float Cellconstant;
  float calibrationRes = 230;

  Cellconstant = (distance/area);
 
  ResWater = RawWaterRes(Vin_avg,avgVoltWater,Rtest) + 230;
  ...
  Serial.print(" RMS Vcb (v): ");
  Serial.print(avgVoltWater,3);
  Serial.print(" - T (ºC): ");
  Serial.print(Celcius);
  Serial.print(" - K (cm-1): ");
  Serial.print(Cellconstant);
  Serial.print(" - Water Resistance (ohm): ");
